...
@synthesize lastSilverQuoteDate;
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict {
    ...
    if ([elementName isEqualToString:@"silver"] || [elementName isEqualToString:@"date"]) {
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName {
    ...
    if ([elementName isEqualToString:@"silver"]) {
        self.lastSilverQuote = [self.currentParsedCharacterData doubleValue];
    }
    if ([elementName isEqualToString:@"date"]) {
        NSDateFormatter *oneShotDateFormatter = [[NSDateFormatter alloc]init];
        [oneShotDateFormatter setDateFormat:@"yyyy-MM-dd"];

        self.lastSilverQuoteDate = [oneShotDateFormatter dateFromString:self.currentParsedCharacterData] ;
    }
...
[[session dataTaskWithURL:[NSURL URLWithString:@"http://services.packetizer.com/spotprices/?f=xml"]
        completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {

            if (error) {
                NSLog(@"error is %@", [error localizedDescription]);
                
                UIAlertView *cantContactServer = [[UIAlertView alloc] initWithTitle:@"Network Error" message:@"Can't update spot price, using last known price. Please check network settings." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
